...
|---|---|---|---|
| `generated_knowledge_references` | `runtime/generated-knowledge/*` primary, `skills/sf6-agent/references/generated-*` compatibility bridge | `packages/knowledge-generation/build-sf6-agent-knowledge.ps1` | `knowledge/curated` |
...
| `frame_current_runtime_assets` | `runtime/frame-current/*` primary, `skills/sf6-agent/assets/frame-current/*` compatibility bridge | `packages/skill-packaging/build-frame-current-runtime-assets.ps1` | `data/exports`, `data/roster` |
| `normalization_runtime_assets` | `runtime/normalization/*` primary, `skills/sf6-agent/assets/normalization/*` compatibility bridge | `packages/skill-packaging/build-normalization-runtime-assets.ps1` | `data/aliases` |
...
`generated_knowledge_references` は現在 `runtime/generated-knowledge/` を primary
runtime output とし、`skills/sf6-agent/references/generated-*` は public adapter が
残る間の compatibility bridge として扱う。hand-written public adapter policy
references とは別責務で扱う。Phase 2 の責務分離 plan は
`docs/architecture/generated-reference-responsibility-plan.md` を参照する。

`frame_current_runtime_assets` は現在 `runtime/frame-current/` を primary runtime output
とし、`skills/sf6-agent/assets/frame-current/` は public adapter が残る間の
compatibility bridge として扱う。移行 plan は
`docs/architecture/frame-current-runtime-separation-plan.md` を参照する。

`normalization_runtime_assets` は現在 `runtime/normalization/` を primary runtime
output とし、`skills/sf6-agent/assets/normalization/` は public adapter が残る間の
compatibility bridge として扱う。
