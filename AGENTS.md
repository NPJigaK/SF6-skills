...
## Deterministic Calculation Tooling / 決定論的計算ツール

- SF6 calculation tools and their tests should use exact arithmetic with SymPy when formulas involve fractions, percentages, scaling, rounding, rates, or symbolic identities.
- SymPy is calculation tooling, not evidence. It must only apply approved formulas to reviewed parsed values; it must not infer SF6 facts from raw text, prose, candidates, or LLM memory.
- Do not add SymPy to raw acquisition, parsing, schema validation, or general validators unless an approved ExecPlan requires numeric calculation in that surface.
