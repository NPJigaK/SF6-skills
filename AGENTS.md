...
- ローカルテキスト検索には `rg` を使う。
- JSON / JSONL から値を取得、集計、検証する場合は `$jq-cli` skill を使い、`jq` CLI を第一選択にする。特に `wiki/outputs/data/`、raw manifest / metadata / validation、frame-data JSON、numeric derived output の数値・row count・条件検証では、ad hoc な text grep や one-off Python より `jq -e` / `jq` filter を優先する。
