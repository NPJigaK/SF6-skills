...
- calculation grounding / exact damage, gauge, juggle, distance, timing proof / fixture, validation,
  prediction, postmortem, rule-promotion decisions:
  `$sf6-calculation-grounding` (`.agents/skills/sf6-calculation-grounding/SKILL.md`)
- JSON / JSONL inspection, validation, aggregation, jq filters, jq quoting:
...
source claim を変更しない backlink repair、index rebuild、deprecated marker 追加、
明らかな duplicate entry 整理は wiki maintenance として実行してよいです。

page merge / split / rename / directory move は semantic 判断を含みやすいため、
明らかな placeholder や完全重複を除き、まず `wiki/reviews/` に refactor plan を作ってください。
...
SF6 の frame、damage、gauge、juggle、distance、timing などの計算 tool は、
source-backed または validation-backed な derived fact だけを扱います。

LLM は route text、community claim、未検証の仮説、source にない内部仕様から、
hit order、状態遷移、距離条件、juggle 条件、補正状態を推測して deterministic output にしてはいけません。

Exact damage / gauge / juggle / distance / timing proof、fixture、prediction、postmortem、
rule promotion、tool contract を扱う時は、`$sf6-calculation-grounding` と
`wiki/concepts/combo-damage-ledger-protocol.md` を使います。

絶対ルール:

- route text から exact value を直接出さない。
- hit order proof または accepted validation なしに ledger を確定しない。
- candidate ledger / candidate fixture を deterministic output に入れない。
- human-only observation を一般 rule / validated_rule に昇格しない。
- community numeric source を official / lab-verified value のように扱わない。
- validation failure の output を source of truth にしない。
- source-backed / validation-backed でない rule を tool 化しない。
- calculator output には source path、authority、validation status、review status、
  calculator version、input hash を残す。
- 不足がある場合は exact answer ではなく candidate ledger、unknowns、required evidence を返す。
...
prediction / postmortem、fixture / regression / validation / rule promotion を含む質問では、
回答前に `$sf6-calculation-grounding` も使ってください。
...
Calculation report、fixture、postmortem、prediction、rule promotion、calculator output を
永続化する場合は、`$sf6-calculation-grounding` の authority / validation /
fixture decision / rule promotion decision を確認してください。

## Calculation Grounding ワークフロー

exact damage、damage / gauge / juggle / distance / timing calculation、source-backed ledger、
fixture、prediction、postmortem、validation、authority、rule promotion、tool contract を扱う場合は:

`$sf6-calculation-grounding` skill を使ってください。skill には calculation evidence lifecycle、
stop condition、fixture / rule promotion、wiki feedback、tool contract 境界の詳細を置きます。

## Refactor ワークフロー

wiki structure refactor、recompile、merge、split、rename、deprecate、hub creation、
index redesign を依頼されたら:

`$sf6-wiki-refactor` skill を使ってください。skill には wiki-only refactor、
refactor trigger、safe refactor、review note、index/log 更新、human-review item 報告の詳細を置きます。

Calculation protocol、damage ledger、prediction / postmortem、fixture lifecycle、tool contract の
再編では `$sf6-calculation-grounding` と `wiki/concepts/combo-damage-ledger-protocol.md` も使ってください。
...
Calculation-related validation failure、candidate fixture 混入、source-backed でない rule promotion、
schema / contract drift は `$sf6-calculation-grounding` の対象として扱ってください。

## Obsidian Markdown

`wiki/` や Obsidian で閲覧する Markdown を作成・編集する場合は、repo-local
`$obsidian-markdown` skill（`.agents/skills/obsidian-markdown/SKILL.md`）を使ってください。
...
- ローカルテキスト検索には `rg` を使う。
- JSON / JSONL から値を取得、集計、検証する場合は repo-local `$jq-cli` skill（`.agents/skills/jq-cli/SKILL.md`）を使い、`jq` / `jq -e` を第一選択にする。特に `wiki/outputs/data/`、raw manifest / metadata / validation、frame-data JSON、numeric derived output の数値・row count・条件検証では、ad hoc な text grep や one-off Python より `jq -e` / `jq` filter を優先する。
