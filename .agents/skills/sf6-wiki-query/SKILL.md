...
3. 不確実性は `unknown`、`needs official source`、`needs source review`、`community-only evidence` のように明示する。
4. source conflict がある場合は黙って片方を選ばず、authority の差を説明し、必要なら review follow-up を残す。
5. 回答が durable なら `$sf6-durable-output` を使うか、その skill の方針に従って file back する。
6. durable write をしない場合も、変更ファイルなしであることと evidence gap を報告する。
