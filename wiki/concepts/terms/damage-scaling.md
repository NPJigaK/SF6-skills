...
Exact damage を問う combo query では、route text をそのまま足し算しない。必ず damaging hit ledger を作る。詳細な preflight は [[concepts/combo-damage-ledger-protocol]] に置く。

- `ODアムネジア`、罠、爆弾、設置、portal、projectile のような delayed hit は、route text で1語しか書かれていなくても複数 hit に展開する。
- `最速入力`、delayed projectile、SA/install、portal、multi-hit、juggle、距離依存がある場合、damage 計算前に hit order proof を作る。source だけで cancel anchor、hitstop、距離、juggle height が足りない場合は、candidate ledger までに留める。
