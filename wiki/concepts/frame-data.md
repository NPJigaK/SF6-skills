...
Frame data は、技の発生、持続、硬直、有利不利、ダメージ、補正、キャンセル可否、Drive gauge 変動などを説明するための timing / property vocabulary と structured move data。現在この wiki には、30 character data slugs 分の Capcom 公式 Classic / Modern frame-data outputs と、SuperCombo Wiki の JP raw community frame-data capture がある。
...
SuperCombo JP capture は、community wiki の `Data?action=raw`、Cargo API、表示 DOM、5タブ別 screenshot、参照画像を保持する。公式 source と重なる基本フレーム値では Capcom 公式 data を優先し、SuperCombo は公式にない range、juggle、notes、hitbox image などを後で統合する候補として扱う。
