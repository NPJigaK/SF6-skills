...
Frame data は、技の発生、持続、硬直、有利不利、ダメージ、補正、キャンセル可否、Drive gauge 変動などを説明するための timing / property vocabulary と structured move data。現在この wiki には、30 character data slugs 分の Capcom 公式 Classic / Modern frame-data outputs と、SuperCombo Wiki の 30 キャラ分 community raw frame-data 取得データがある。raw frame-data の現在の入口は latest mirror 固定パスで、SuperCombo の source freshness は manifest の `source_updated_at` / `source_revision` から、raw 取得時刻は `captured_at_utc` から確認する。
