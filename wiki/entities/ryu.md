...
Ryu（リュウ） は、この wiki で Capcom 公式フレームデータ source と SuperCombo Wiki community frame-data source が保存されている Street Fighter 6 character。公式 frame-data table の data slug は `ryu`。
...
- capture date / source revision などの provenance は [[sources/capcom-official-ryu-frame-data]] と [[sources/supercombo-ryu-frame-data]] から辿る。
- 入力 icon や Modern control token は `input_tokens` に raw token sequence として保持されている。
- 個別技の値は entity page では要約せず、`wiki/outputs/data/frame-data/official/ryu/classic.json` / `wiki/outputs/data/frame-data/official/ryu/modern.json` の公式 JSON output と source page を source-preserving data として参照する。
