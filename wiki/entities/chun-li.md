---
type: entity
entity_type: character
created: 2026-05-27
updated: 2026-05-27
status: active
sources:
  - "[[sources/capcom-official-chun-li-frame-data]]"
related:
  - "[[entities/street-fighter-6]]"
  - "[[concepts/frame-data]]"
tags:
  - character
  - frame-data
---

# Chun-Li

## Summary

Chun-Li is the Street Fighter 6 character covered by the third official
frame-data capture in this wiki.

## Timeline

| Date | Event | Source |
|---|---|---|
| 2026-05-27 | Official Chun-Li frame-data page captured for Classic and Modern controls. | [[sources/capcom-official-chun-li-frame-data]] |

## Relevant claims

- The official capture contains 78 Classic frame-data rows and 72 Modern
  frame-data rows.
- The capture keeps raw input token data, including controller icons and
  Modern-control tokens.
- This page does not yet summarize individual move values; those remain in the
  derived CSVs and raw DOM artifacts.
- The capture was reviewed and accepted by the human reviewer.

## Related concepts

- [[concepts/frame-data]]
- [[concepts/fighting-game-notation]]

## Open questions

- Which Chun-Li move values should be compiled into durable wiki summaries
  after human review?
