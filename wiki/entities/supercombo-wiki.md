...
SuperCombo Wiki は、この wiki が最初に ingest した Street Fighter 6 glossary source、controls source、30 キャラ分の frame-data community source の掲載元。glossary は Drive System、notation、juggle、frame-data terms の補助 source として [[concepts/terms/index]] に統合され、controls は numpad notation、Classic / Modern control type、button notation の補助 source として [[concepts/fighting-game-notation]] に統合されている。公式 Classic data を正とする SuperCombo 補助列付き output も 30 キャラ分作成済み。
