...
Mai（不知火舞） は、この wiki で Capcom 公式フレームデータ capture が保存されている Street Fighter 6 character。公式 frame-data table の data slug は `mai`。
...
- 個別技の値は entity page では要約せず、`wiki/outputs/data/frame-data/mai/` の CSV と raw DOM を source-preserving data として参照する。
...
- Mai（不知火舞） のどの技データを、再利用できる質問・統合分析として wiki に昇格すべきか。
