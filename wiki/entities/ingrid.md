...
Ingrid（イングリッド） は、この wiki で Capcom 公式フレームデータ source、SuperCombo community frame-data source、公式 Classic + SuperCombo 補助データが保存されている Street Fighter 6 character。公式 frame-data table の data slug は `ingrid`。
...
- SuperCombo source は 1 件の character template、83 件の frame template、20 件の DOM table comparison を含む。
- SuperCombo source は imageinfo で face / portrait の 2 件だけ解決でき、156 title は missing だった。
...
- 個別技の値は entity page では要約せず、`wiki/outputs/data/frame-data/official/ingrid/classic.json` / `wiki/outputs/data/frame-data/official/ingrid/modern.json` の公式 JSON output、`wiki/outputs/data/frame-data/official-supercombo-enriched/ingrid/classic-supercombo.json` / `wiki/outputs/data/frame-data/official-supercombo-enriched/ingrid/supercombo-only.json` の補助列付き JSON output、source page を source-preserving data として参照する。
- SuperCombo 補助データのレビュー対象 26 行は人間レビュー済み accepted。capture review は imageinfo missing と SuperCombo-only 行の扱いが残るため open。
