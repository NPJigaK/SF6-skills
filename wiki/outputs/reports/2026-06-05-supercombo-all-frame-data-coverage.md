...
SuperCombo 派生 output には `frames.json`、`character.json`、`crosswalk-official-classic.json`、`crosswalk-summary.json`、`supercombo-unmatched.json`、`schema.json` がある。

公式 + SuperCombo 補助 output には `classic-supercombo.json`、`supercombo-only.json`、`summary.json`、`schema.json` がある。

## レビュー状態

JP / Ryu / Zangief / Ingrid は既存の作業で一部補助リンクを人間レビュー済みにしており、accepted 69 行は `enriched_reviewed` として保持している。2026-06-06 の fail-closed policy 以降は、複数候補、SuperCombo row 再利用、基本 field conflict、比較不能 field、条件付き SuperCombo field を持つ行を `enriched_review_required` に落とす。`着地後N` と `N land` のような landing recovery 表記差と、`400x2` や `300,400` のような括弧・注記なしの多段 damage 合計だけを機械正規化した。damage / startup / recovery の括弧付き条件値は `condition_dependent_supercombo_field` として review queue に残す。未レビュー補助列は 30 キャラ全体で 1295 行。

| 分類 | 対象 | 状態 |
|---|---|---|
| raw capture | 30 キャラ | 自動検証 `passed`。 |
| 既存 review 済み補助列 | JP / Ryu / Zangief / Ingrid | 既存の `human_review_status: accepted` 69 行を保持。 |
| 未レビュー補助列 | 30 キャラ | `enriched_review_required` 1295 行。勝手に accept していない。 |
...
`classic-supercombo.json` には、review flags を用途別にまとめた `enrichment_review_queues` を追加した。これは人間レビューを分割するための補助 field であり、キューが付いている行を自動 accept する意味ではない。

| queue | 件数 | 意味 |
|---|---:|---|
| `uncomparable_notation` | 1138 | 現在の機械比較では安全に数値化できない notation を含む。 |
| `structural_ambiguity` | 851 | 複数候補または SuperCombo row 再利用がある。 |
| `condition_dependent_field` | 554 | 括弧付き値など、条件で値が変わる SuperCombo field を含む。 |
| `field_conflict` | 199 | 公式値と SuperCombo の基本 field が衝突している。 |
| `manual_or_ambiguous_match` | 161 | 既存 override または曖昧照合に依存している。 |

代表的な組み合わせは `uncomparable_notation;structural_ambiguity` 316 行、`condition_dependent_field;uncomparable_notation` 251 行、`condition_dependent_field;uncomparable_notation;structural_ambiguity` 189 行。次のレビューでは、単独 queue から始めると判断単位を小さくできる。
...
- SuperCombo は community source なので、公式 data と重なる基本フレーム値では Capcom 公式 Classic JSON を正とする。
...
- enriched summaries: `wiki/outputs/data/frame-data/official-supercombo-enriched/<character_slug>/summary.json`
