...
Capcom 公式サイトの Marisa（マリーザ） フレームデータを、Classic / Modern の raw HTML、DOM、screenshot、metadata として保存し、JSON に派生抽出した source page。
...
| 派生データは Classic 91 行、Modern 80 行。 | `raw/frame-data/official/marisa/manifest.json`; `wiki/outputs/data/frame-data/official/marisa/classic.json`; `wiki/outputs/data/frame-data/official/marisa/modern.json` | high | JSON `row_count` は raw DOM 由来の table row count と対応する。 |
| field meanings は各 control JSON の `field_meanings.records` に保存されている。 | `wiki/outputs/data/frame-data/official/marisa/classic.json`; `wiki/outputs/data/frame-data/official/marisa/modern.json` | high | Classic 7 records、Modern 8 records。 |
...
- Derived Classic JSON: `wiki/outputs/data/frame-data/official/marisa/classic.json`
- Derived Modern JSON: `wiki/outputs/data/frame-data/official/marisa/modern.json`
