...
| Drive Rush from Parry | cost table は transition component として `1/2` Drive Stock 相当の icon を示す。Drive Rush section / caption text の `1` Drive Stock は、Drive Parry activation `1/2` 込みの最小総消費として扱う。 |
...
- Drive Rush from Parry は cost table の `1/2` を transition component、section / caption text の `1 Drive Stock` を Drive Parry activation 込みの最小総消費として扱う。derived JSON は PDR の `cost_bars` を単一 direct cost として使わず、raw table 値、transition component、section text 値、minimum total を別 field で保持する。
...
- Drive Rush from Parry の exact frame / input buffer と、frame `4` の hold drain が特定入力で何 frame 分乗るかをどう検証するか。
