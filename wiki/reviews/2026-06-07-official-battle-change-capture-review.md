...
- `tools/extract_capcom_battle_change.py` は raw JSON から `changes.csv`、`changes.json`、`versions.csv`、`schema.json` を生成した。
...
- `changes.csv` の `text_html` は公式 HTML fragment を保持する。これは raw 由来の派生列であり、翻訳・要約・正規化した source replacement ではない。
