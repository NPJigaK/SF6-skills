...
- SuperCombo 30キャラ分は派生 output / official crosswalk / 補助列付き output まで作成済み。既存レビュー済み 69 行は保持し、複数候補、SuperCombo row 再利用、基本 field conflict、比較不能 field、条件付き SuperCombo field を持つ 1296 行は `enriched_review_required` として accept していない。
