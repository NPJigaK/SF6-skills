...
frame-data raw は、capture date をディレクトリ名に入れる方式ではなく、`raw/frame-data/official/<character>/` と `raw/frame-data/supercombo/<character>/` の最新ミラー固定パスで保持する。`storage_policy: latest_frame_data_mirror` を持つ更新可能な raw 一式なので、再取得時は同じ path の原文・元データを保つ取得物、manifest、metadata、validation、hash を更新する。由来の時点は path ではなく、各 `manifest.json` の `capture_label`、`created_at_utc` / `captured_at_utc`、SuperCombo の `source_updated_at` / `source_revision` で追う。2026-06-06 以降、SuperCombo の `validation.json` は現在の raw metadata と実ファイル hash に対応する `raw_fingerprint` も持ち、extract は fingerprint 不一致の validation を拒否する。2026-06-07 以降、派生 output は `wiki/outputs/data/frame-data/official/`、`wiki/outputs/data/frame-data/supercombo/`、`wiki/outputs/data/frame-data/official-supercombo-enriched/` の data-family first layout で置く。2026-06-05 時点で、official は 30 character data slugs、SuperCombo は 30 character slugs の raw capture と派生 output がそろっている。

この方針により、読者は常に同じ raw path から最新ミラーを辿り、再現性や履歴判断が必要な場合だけ manifest の由来を示す項目を見る。`raw/` は原本保存層なので、最新ミラーでも翻訳、要約、正規化、統合分析は `wiki/` に置く。
...
| Source family | Manifest | 主要 raw 取得物 | Derived outputs |
|---|---|---|---|
...
| Field | Applies to | 意味 |
|---|---|---|
| `storage_policy` | official / SuperCombo | `latest_frame_data_mirror` の場合、raw path は固定の最新ミラーで、capture date を path に含めない。再取得時は同じ raw 一式の原文・元データを保つ取得物と、由来を示す項目を更新する。 |
| `capture_label` | official / SuperCombo | snapshot の由来を表すラベル。具体的な日付は各 manifest で確認する。SuperCombo は 2026-06-05 時点で 30キャラ分の最新ミラーを持つ。 |
...
| `raw_fingerprint` | SuperCombo validation | `metadata.json` と raw 取得物の実ファイルの現在状態に validation が対応していることを示す。capture 後に古い validation が残る状態を extract 前に拒否するために使う。 |
...
- 既存 raw manifest の `derived_outputs` に旧 output path が残っていても、別途の再取得または raw 一式の保守作業でない限り、query や wiki 更新のついでに raw manifest だけを書き換えない。現在の output contract は README、tools、`wiki/index.md`、この synthesis を入口に確認する。
...
- `latest_frame_data_mirror` の再取得時に旧ミラーをどこまで外部保管物として保持するか。
