# Workflows

`workflows/` contains canonical maintainer procedures.

Workflow docs are readable by humans and agents without requiring a skill runtime.

## Canonical Procedures

- [GitHub management](github-management.md)
- [Manage maintainer toolchain](manage-maintainer-toolchain.md)
- [Maintainer agent session](maintainer-agent-session.md)
- [Codex-to-Hermes delegation](codex-to-hermes-delegation.md)
- [Hermes ingest profile setup](hermes-ingest-profile-setup.md)
- [Media scratch cache policy](media-scratch-cache-policy.md)
- [System mechanics fact workflow](system-mechanics-fact-workflow.md)
- [Update frame data](update-frame-data.md)
- [Ingest article](ingest-article.md)
- [Review claims](review-claims.md)
- [Ingest video](ingest-video.md)
