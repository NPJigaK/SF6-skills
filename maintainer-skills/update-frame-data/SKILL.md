...
description: Use when a maintainer asks to refresh the supported baseline character frame-data (`jp`, `luke`) after an official balance patch, battle change list update, or frame-page change in this repository. Use for the full maintainer workflow: verify the official patch label/date first, run ingest, rebuild published runtime assets, prune backing raw snapshots, verify outputs, and prepare accurate reporting or commit wording without guessing patch metadata.
---
...
- Use this when the user says things like `調整入ったから更新して`, `JP/Luke のデータ更新して`, or `バトル変更リスト対応で更新して`.
...
3. Run end-to-end ingest for `jp`.
4. Run end-to-end ingest for `luke`.
5. Read `data/exports/jp/snapshot_manifest.json` and `data/exports/luke/snapshot_manifest.json`.
6. Record each dataset state separately: `published`, `retained_last_known_good`, or `unavailable`.
...
python -m sf6_ingest.cli prune --character jp --dry-run --verbose
python -m sf6_ingest.cli prune --character luke --dry-run --verbose
python -m sf6_ingest.cli prune --character jp --apply --verbose
python -m sf6_ingest.cli prune --character luke --apply --verbose
```
...
Read the full output and confirm exit code `0` for all three.

## Reporting Rules

- Report `jp` and `luke` separately.
...
- Distinguish official-published updates from enrichment-only updates.
...
- Prefer the exact official label when available, for example:
  - `2026.03.17 updateのバトル変更リスト対応でJP/Lukeのフレームデータを更新`
  - `Ver.2.300のバトル変更リスト対応でJP/Lukeのフレームデータを更新`
...
- Reporting both characters as fully updated when one dataset stayed on last-known-good
